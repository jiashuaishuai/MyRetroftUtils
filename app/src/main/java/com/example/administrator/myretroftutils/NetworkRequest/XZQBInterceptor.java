 ...
 */
public class XZQBInterceptor implements Interceptor {
    ...
    public XZQBInterceptor() {
        this(Logger.DEFAULT);
    }

    public XZQBInterceptor(Logger logger) {
        this.logger = logger;
    }
    ...
    public XZQBInterceptor setLevel(Level level) {
                ...
                if (contentLength != 0) {
                    ...
                    try {
                        ...
                        if (data.startsWith("{\"data\":\"")) {
                            //说明有可能是加密数据的结构
                            logger.log(request.url() +"-->"+decryptCode(data.substring(8,data.length()-2)));
                        }
                    }catch(Exception e){
